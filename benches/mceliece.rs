...
use kem_combiners::{
    combiners::*,
    mceliece::{self, DecapsulationKey, EncapsulationKey, NewMcEliece},
    Combiner,
};
...
fn bench_combiner<C: Combiner>(
    ...
    let (ct, ss) = mceliece::encap(combo, &mut rng, &ek);
    ...
    c.bench_function(&format!("{}_encap", label), |b| {
        b.iter(|| {
            mceliece::encap(combo, &mut rng, &ek);
        })
    });

    c.bench_function(&format!("{}_decap", label), |b| {
        b.iter(|| {
            mceliece::decap(combo, &dk, &ct);
        })
    });
}
...
pub fn criterion_benchmark(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    let (dk, ek) = mceliece::generate(&mut rng);
