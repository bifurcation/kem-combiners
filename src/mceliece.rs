...
mod mceliece {
    pub type EncapsulationKey = classic_mceliece_rust::PublicKey<'static>;
    pub type DecapsulationKey = classic_mceliece_rust::SecretKey<'static>;
    pub type Ciphertext = classic_mceliece_rust::Ciphertext;
}

pub struct DecapsulationKey {
    pub t: x25519_dalek::StaticSecret,
    pub pq: mceliece::DecapsulationKey,
    pub ek: EncapsulationKey,
}

pub struct EncapsulationKey {
    pub t: x25519_dalek::PublicKey,
    pub pq: mceliece::EncapsulationKey,
}
...
pub struct Ciphertext {
    pub t: x25519_dalek::PublicKey,
    pub pq: mceliece::Ciphertext,
}

pub trait NewMcEliece {
    fn new_mceliece(ek: &EncapsulationKey) -> Self;
}

impl<T> NewMcEliece for T
where
    T: NewPre,
{
    fn new_mceliece(ek: &EncapsulationKey) -> Self {
        Self::new_pre(ek.t.as_bytes().as_slice(), ek.pq.as_ref())
    }
}
...
pub fn generate(rng: &mut impl CryptoRngCore) -> (DecapsulationKey, EncapsulationKey) {
    ...
    let (ek_pq, dk_pq) = classic_mceliece_rust::keypair_boxed(&mut *rng);
...
pub fn encap<C: Combiner>(
    ...
    let (ct_pq, ss_pq) = classic_mceliece_rust::encapsulate_boxed(&ek.pq, &mut *rng);
...
pub fn decap<C: Combiner>(c: &C, dk: &DecapsulationKey, ct: &Ciphertext) -> SharedSecret {
    ...
    let ss_pq = classic_mceliece_rust::decapsulate_boxed(&ct.pq, &dk.pq);
...
mod test {
    ...
    // Key generation takes *forever*, so we do it once and cache the results.
    static KEY_PAIR: OnceLock<(DecapsulationKey, EncapsulationKey)> = OnceLock::new();

    fn key_pair() -> &'static (DecapsulationKey, EncapsulationKey) {
    ...
    fn kitchen_sink_pre() {
        let (dk, ek) = key_pair();
        let kitchen_sink_pre = KitchenSinkPre::new_mceliece(&ek);
    ...
    fn chempat_pre() {
        let (dk, ek) = key_pair();
        let chempat_pre = ChempatPre::new_mceliece(&ek);
    ...
    fn dhkem_pre() {
        let (dk, ek) = key_pair();
        let dhkem_pre = DhkemPre::new_mceliece(&ek);
